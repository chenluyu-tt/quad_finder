...
def load_example_database():
    """
    Load example hands generated by the random search program.
    """
    try:
...
def get_available_k_values(example_data):
    """
    Return k values that actually have examples.
    """
    ...
    for k_str in example_data.keys():
        examples = example_data[k_str].get("examples", {})
        if examples:
...
def get_available_quad_counts(example_data, k):
    """
    For a chosen k, return available quad counts.
    """
    k_str = str(k)
    ...
    if k_str not in example_data:
        return []

    examples = example_data[k_str].get("examples", {})

    return sorted([int(q_str) for q_str in examples.keys()])


def get_examples_for_choice(example_data, k, q):
    """
    Return the list of examples for a chosen k and quad count q.
    """
    k_str = str(k)
    q_str = str(q)
...
st.sidebar.markdown("---")
st.sidebar.subheader("Load Saved Example")

example_data = load_example_database()

if example_data is None:
    st.sidebar.warning(
        "Example JSON file not found. "
        f"Make sure {EXAMPLE_FILE} is in the same folder as this app."
    )
else:
    available_k_values = get_available_k_values(example_data)

    if not available_k_values:
        st.sidebar.warning("No examples found in the JSON file.")
    else:
        k_choice = st.sidebar.selectbox(
            "Choose number of cards k",
            available_k_values,
            index=0,
        )

        available_q_values = get_available_quad_counts(example_data, k_choice)

        q_choice = st.sidebar.selectbox(
            "Choose number of quads",
            available_q_values,
            index=0,
        )

        example_list = get_examples_for_choice(example_data, k_choice, q_choice)

        example_numbers = list(range(1, len(example_list) + 1))
        ...
        selected_example = example_list[example_choice - 1]

        st.sidebar.caption(
            f"k = {k_choice}, quads = {q_choice}, "
            f"example {example_choice}/{len(example_list)}"
        )

        if st.sidebar.button("Load This Example"):
